...
### Advanced Finetuning Modes

### Partial Finetuning
You can unfreeze specific layers of the vision encoder or LLM while keeping the rest frozen.
Modify `config.py`:
```python
@dataclass
class ModelConfig:
    # ...
    tune_visual_layers: int = 2  # Unfreeze last 2 layers of vision encoder
    tune_llm_layers: int = 2     # Unfreeze last 2 layers of LLM
```

### LoRA Finetuning
You can use LoRA (Low-Rank Adaptation) to finetune the model efficiently.
Modify `config.py`:
```python
@dataclass
class ModelConfig:
